...
class SenderTracker:
    ...
    def _get_state_unlocked(self, sender: str) -> SenderState:
        """Get current state for a sender (no lock — internal use only)."""
    ...
    def is_allowed(self, sender: str) -> bool:
        ...
        with self._lock:
            state = self._get_state_unlocked(sender)
